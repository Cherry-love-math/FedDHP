...
class FedGen(Server):
    ...
    def _payload_bytes_model(self, model):
        sd = model.state_dict()
        total = 0
        for v in sd.values():
    ...
    def send_models(self):
        ...
        per_client = self._payload_bytes_model(self.global_model) + self._payload_bytes_model(self.generative_model)
        ...
        for client in active_clients:
            ...
            if client_time_cost <= self.time_threthold:
                ...
                if self.localize_feature_extractor:
                    ...
                    up += self._payload_bytes_model(client.model.head)
                else:
                    self.uploaded_models.append(client.model)
                    up += self._payload_bytes_model(client.model)
